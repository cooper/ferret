...
our %pretty_tokens = (
    ...
    OP_BAND     => [ '&',   'bitwise and operator (&)'                      ],
    OP_XOR      => [ '^^',  'bitwise exclusive or operator (^^)'            ],
    OP_BOR      => [ '|',   'bitwise or operator (|)'                       ],
    ...
    OP_POW_A    => [ '^=',  'altering power operator (^=)'                  ],
    OP_BAND_A   => [ '&=',  'altering bitwise and operator (&=)'            ],
    OP_XOR_A    => [ '^^=', 'altering bitwise exclusive or operator (^^=)'  ],
    OP_BOR_A    => [ '|=',  'altering bitwise or operator (|=)'             ],
    OP_EQUAL_I  => [ '===', 'reference equality operator (===)'             ],
    OP_EQUAL    => [ '==',  'equality operator (==)'                        ],
    OP_SIM      => [ '=~',  'similarity operator (=~)'                      ],
    OP_NEQUAL_I => [ '!==', 'negated reference equality operator (!==)'     ],
    OP_NEQUAL   => [ '!=',  'negated equality operator (!=)'                ],
    OP_NSIM     => [ '!~',  'negated similarity operator (!~)'              ],
    ...
    OP_GR8R_E   => [ '>=',  'greater than or equal to operator (>=)'        ],
    OP_LESS     => [ '<',   'less than operator (<)'                        ],
    OP_GR8R     => [ '>',   'greater than operator (>)'                     ],
    ANGLE_S     => [ '<',   'opening angle or less than operator (<)'       ],
    ANGLE_E     => [ '>',   'closing angle or greater than operator (>)'    ],
    PROP_VALUE  => 'property key (property: ...)',
    BAREWORD    => 'bareword',
...
sub pretty_token {
    ...
    $tok = $pretty_tokens{$tok} || $tok;
    return ref $tok ? $tok->[1]  : $tok;
}

sub operator_token {
    my $tok = shift;
    $tok = $pretty_tokens{$tok} || $tok;
    return ref $tok ? $tok->[0]  : $tok;
}
